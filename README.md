...
- **独立 socket 通道**：文件传输与目录浏览使用 `control_port + 1` 的 file channel，避免大文件影响实时画面与输入控制。
- **双栏传输弹窗**：左侧远程目录、右侧本机目录，支持盘符浏览、目录进入/返回、文件类型图标显示。
- **分片传输与断点续传**：默认 256KB 分片，支持暂停/继续，传输状态持久化到 `logs/transfer_state.json`，掉线后自动续传。
- **任务列表增强**：显示进度、状态、实时速度，支持右键暂停/继续/取消任务。
- **文件管理增强**：远程与本机目录树均支持右键删除文件/目录（目录递归删除，含二次确认）。
- **交互体验**：传输窗口为应用级模态对话框，减少并发操作导致的状态冲突。
